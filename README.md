...
>
> **皮肤模式与应用设置**：控制台支持浅色 / 深色 / 跟随系统三态皮肤（侧边栏底部「皮肤」按钮循环切换，应用设置 → 外观可精确选择）；「应用设置」入口 —— 外观、网络代理、重置访问密码、重置运行环境/修复服务（参考 [yuexps/deepseek.harness.fnos](https://github.com/yuexps/deepseek.harness.fnos) 应用设置模块）；「检查更新/升级」位于控制台首页「快速操作」模块。
>
...
- **QwenPaw 控制台** — 带侧边栏的服务管理控制台：
  ...
  - **快速操作** — 打开 QwenPaw 完整界面 / 跳转运行日志 / 检查更新（查询 GitHub Releases 最新版本，支持经网络代理检查；发现新版本后引导在 fnOS 应用中心安装新版 `.fpk`，数据与配置保留）
  ...
  - **飞牛统一网关**（侧边栏底部）— 单端口 HTTP/HTTPS 自适应反向代理：
    ...
    - **三种打开方式** — 飞牛统一网关（经 fnOS 统一网关 `/app/qwenpaw_yuexps/qwenpaw/` 子路径，支持外网）/ 反代端口（直连代理端口）/ 自定义地址（填外部反向代理域名后跳转）
    - **自定义外部地址** — 支持 `http(s)://` 前缀，方便接入已有反代域名
    - **访问地址** — 展示飞牛统一网关入口（HTTP 5666 / HTTPS 5667）与反代端口局域网地址，一键打开
  - **应用设置**（侧边栏底部）— 参考 DHS 应用设置模块：
    - **外观** — 皮肤模式三态：浅色 / 深色 / 跟随系统（localStorage 持久化，侧边栏底部按钮可循环切换）
